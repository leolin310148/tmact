...
Usage:
  ...
  tmact session resume SESSION --dir DIR --agent claude|codex --session-id ID [--execute] [--json]
  ...
  tmact log stats [--since 24h] [--json]
  tmact log doctor [--json]
...
Commands:
  ...
  session       create, close, reopen, and explicitly resume guarded local sessions
  ...
  log           privacy-safe search, statistics, and health for Claude / Codex logs
