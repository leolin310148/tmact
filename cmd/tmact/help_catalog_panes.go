	...
	return []commandHelp{
		...
		},
		{
			...
			Flags: []helpFlag{
				...
				{Name: "--no-clear", Description: "when reusing an idle same-agent pane, continue its conversation instead of sending /clear first; local-only"},
			...
			Examples: []string{
				...
				`tmact dispatch-work work --dir . --agent claude --prompt "now address the failing test" --no-clear --execute`,
		...
		},
		{
			...
			Flags: []helpFlag{
				...
				{Name: "--json", Description: "print the question ID, delivery, dispatch report, and reply as JSON"},
			},
			Examples: []string{
			...
			Safety: []string{
				...
				"Reply delivery uses a private local mailbox; it never injects keys into the waiting asker's pane.",
				"A follow-up reaches the answerer through the mailbox only while it is blocked in `tmact reply --wait`; otherwise it is dispatched into the recorded pane without /clear, subject to the same idle, prompt, and lease checks as dispatch-work.",
		...
		{
			Command: "reply",
			Summary: "Return an answer to the local asker waiting on a question ID; optionally wait for its follow-up or close the question.",
			Usage: []string{
				"tmact reply QUESTION_ID --text TEXT [--wait [--timeout 10m] | --final] [--store-dir DIR] [--json]",
				"tmact reply QUESTION_ID --file PATH [--wait [--timeout 10m] | --final] [--store-dir DIR] [--json]",
			},
			Flags: []helpFlag{
				...
				{Name: "--wait", Description: "after replying, block until the asker sends a follow-up and print it; use for clarifying questions you need answered before you can finish"},
				{Name: "--timeout", Value: "DURATION", Description: "max wait for the asker's follow-up with --wait; default 10m, keep it short because it occupies your tool call"},
				{Name: "--final", Description: "mark the reply final and close the question; mutually exclusive with --wait"},
			...
			Examples: []string{
				...
				`tmact reply q_abcdefghijklmnopqrstuvwxyz --text "which branch should I target?" --wait --timeout 5m`,
				`tmact reply q_abcdefghijklmnopqrstuvwxyz --text "merged and verified" --final`,
			},
			Safety: []string{
				"Run reply exactly once per turn. Late, expired, closed, unknown, and malformed IDs are rejected; a duplicate on an open question is accepted as another message, so do not resend.",
				"`reply` writes only to the ask mailbox and never sends tmux keys.",
				"A --wait timeout does not close the question; finish the turn normally and the asker can still continue in your pane.",
			},
			Notes: []string{
				"The random question ID is a local capability. Use only the ID and optional --store-dir supplied in the dispatched prompt.",
				"The asker's follow-up arrives through the mailbox while you are in --wait, and through your pane as a new prompt otherwise; answer each follow-up with reply again.",
