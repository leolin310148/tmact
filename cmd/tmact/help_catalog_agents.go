...
func agentCommandHelpCatalog() []commandHelp {
	return []commandHelp{
		...
		},
		{
			...
			Examples:    []string{"tmact statusd once --json", "tmact statusd start --interval 1s"},
		...
		},
		{
			...
			Usage:    []string{"tmact statusd once [--json] [--initial-samples 2]"},
			Flags:    statusdHelpFlags(),
			Examples: []string{"tmact statusd once", "tmact statusd once --json"},
		},
		{
			Command: "statusd read",
			Summary: "Fetch the latest statusd snapshot from the running daemon (falls back to a live scan if the daemon is down).",
			Usage:   []string{"tmact statusd read [--json] [--socket-path PATH]"},
			Flags: []helpFlag{
				{Name: "--socket-path", Value: "PATH", Description: "daemon IPC unix socket"},
				{Name: "--json", Description: "print JSON output"},
			},
			Examples: []string{"tmact statusd read", "tmact statusd read --json"},
		},
		{
			Command: "statusd status",
			Summary: "Print statusd snapshot freshness and summary counts.",
			Usage:   []string{"tmact statusd status [--json] [--socket-path PATH]"},
			Flags: []helpFlag{
				{Name: "--socket-path", Value: "PATH", Description: "daemon IPC unix socket"},
				{Name: "--json", Description: "print JSON output"},
			},
		...
		},
		{
			...
			Usage:   []string{"tmact panels plan [--config examples/agents.yaml] [--agent NAME] [--role ROLE] [--session SESSION] [--trust-folders] [--json]"},
			Flags: append(agentFilterHelpFlags(),
				helpFlag{Name: "--session", Value: "SESSION", Description: "override target tmux session for selected agents"},
				helpFlag{Name: "--trust-folders", Description: "show exact-repo trust handling in the plan for Claude/Codex"},
			),
		...
		},
		{
			...
			Usage:   []string{"tmact panels ensure [--config examples/agents.yaml] [--agent NAME] [--role ROLE] [--session SESSION] [--trust-folders] [--trust-timeout 30s] [--execute] [--json]"},
			Flags: append(agentFilterHelpFlags(),
				helpFlag{Name: "--session", Value: "SESSION", Description: "override target tmux session for selected agents"},
				helpFlag{Name: "--execute", Description: "apply planned tmux panel changes"},
				helpFlag{Name: "--trust-folders", Description: "accept exact-repo Claude/Codex trust prompts for newly created panes"},
				helpFlag{Name: "--trust-timeout", Value: "DURATION", Description: "maximum wait per opted-in pane; default 30s"},
			),
			Examples: []string{"tmact panels ensure --session sample-team", "tmact panels ensure --session sample-team --trust-folders --execute"},
			Safety:   []string{"Without --execute this prints the planned changes and does not touch tmux.", "Folder trust is opt-in by --trust-folders or per-agent trust_folder: true, supports only Claude/Codex, and requires exact repo/pane-cwd equality."},
		},
	}
}
...
func statusdHelpFlags() []helpFlag {
	return []helpFlag{
		...
		{Name: "--no-tmux-options", Description: "skip writing @ai-* tmux options"},
