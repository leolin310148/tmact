...
func buildLLMInstructions() llmInstructions {
	return llmInstructions{
		...
		RecommendedWorkflow: []string{
			...
			"When the caller needs a definitive result from another local agent session, use `tmact ask SESSION --dir DIR --agent AGENT --prompt TEXT --timeout DURATION --execute --json` instead of shell polling. The dispatched prompt tells the answerer to call `tmact reply QUESTION_ID`; ask blocks until that one-shot reply and returns it on stdout. Do not infer an answer from pane state or exit code.",
