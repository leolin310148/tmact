...
func buildLLMInstructions() llmInstructions {
	return llmInstructions{
		...
		RecommendedWorkflow: []string{
			...
			"Preview side-effecting commands first. `send`, `session close`, `session reopen`, `broadcast`, `panels ensure`, `workflow`, and `dispatch-work` are dry-run or planning-oriented until `--execute` is supplied.",
		...
		JSONTips: []string{
			...
			"`ls --json`, `capture --json`, `wait --json`, `session closed --json`, `inspect --json`, `statusd read --json`, `usage --json`, and workflow status/report commands are preferred for machine parsing.",
