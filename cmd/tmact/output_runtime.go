...
func printRuntimeStatuses(statuses []runmeta.Status) {
	...
	fmt.Fprintln(writer, "id\tstatus\tdesired\tphase\tmode\tpid\ttarget\trun-dir\tconfig\tlast")
	for _, status := range statuses {
		...
		fmt.Fprintf(writer, "%s\t%s\t%s\t%s\t%s\t%d\t%s\t%s\t%s\t%s\n", run.ID, status.RuntimeStatus, status.DesiredState, displayPhase(run.Phase), loopRunMode(run), run.PID, run.Target, status.RunDir, run.ConfigPath, last)
		if run.Tmux.PaneID != "" {
