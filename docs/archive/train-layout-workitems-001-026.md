# Archive — Train Layout Work Items TRAIN-001–026

These completed work items established the infinite moving world, deterministic
route, parallax renderer, initial scenery kit, regions, time palettes, motion,
set pieces, station behavior, train/track integration, and the first visual
polish pass. They are archived so active queue workers only need to read the
current redesign work.

The active queue is [WORKITEM.md](../../WORKITEM.md).
