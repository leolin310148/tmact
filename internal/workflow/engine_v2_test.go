...
func TestStoreRejectsStaleWriterAndRunnerLockIsExclusive(t *testing.T){loaded:=loadTestWorkflow(t,minimalConfig("","  - {id: ok, type: command, argv: [/usr/bin/true]}\n"));engine,err:=NewEngine(loaded,filepath.Join(t.TempDir(),"runs"),true);if err!=nil{t.Fatal(err)};stale,err:=engine.Store.Read();if err!=nil{t.Fatal(err)};if err:=engine.Store.Update(func(s *State)error{s.Status="paused";return nil});err!=nil{t.Fatal(err)};stale.Status="running";if err:=engine.Store.Write(stale);!errors.Is(err,ErrStateConflict){t.Fatalf("error=%v",err)};release,err:=engine.Store.AcquireRunnerLock();if err!=nil{t.Fatal(err)};defer release();if _,err:=engine.Store.AcquireRunnerLock();err==nil{t.Fatal("duplicate runner lock succeeded")}}
